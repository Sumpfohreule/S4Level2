...
aggregateClimateData <- function(plot) {
    ...
    climate_data <- freiland_import %>%
        ...
        mutate(variable = MyUtilities::remapLevels(variable, pattern = "^Lufttemperatur$", replacement = "AT")) %>%
        mutate(variable = MyUtilities::remapLevels(variable, pattern = "^Precipitation|Niederschlag.Casella$", replacement = "PR")) %>%
        mutate(variable = MyUtilities::remapLevels(variable, pattern = "^Luftfeuchte$", replacement = "RH")) %>%
        mutate(variable = MyUtilities::remapLevels(variable, pattern = "^SolarRadiation$", replacement = "SR")) %>%
        mutate(variable = MyUtilities::remapLevels(variable, pattern = "^Windgeschwindigkeit", replacement = "WS")) %>%
